...
void ofApp::update(){
	...
	if(realSense->update()) {
		...
		realSense->updatePointCloud(realSense->getRgbFrame());
		/*
...
void ofApp::draw(){
    ...
    if(bShowVisuals){
		...
		realSense->drawDepth(viewGrid[0]);
		realSense->drawColor(viewGrid[1]);
        ...
        switch (iMainCamera) {
            case 0:
				realSense->drawDepth(viewMain);
                drawCalibrationPoints();
                break;
            case 1:
				realSense->drawColor(viewMain);
                drawCalibrationPoints();
                break;
            case 2:
